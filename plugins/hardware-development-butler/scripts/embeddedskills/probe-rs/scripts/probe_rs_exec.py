...
def main() -> None:
    ...
    gate = safety_gate.check_token(
        args.action,
        args.confirm_token,
        target=params["chip"] or "",
        probe=params["probe"] or "probe-rs",
        voltage=args.voltage,
        current_limit=args.current_limit,
        erase_scope=args.erase_scope or ("chip-erase" if getattr(args, "chip_erase", False) else ""),
        recovery=args.recovery,
        external_loads=args.external_loads,
        artifact=args.safety_artifact or params["file"] or "",
    ...
    if not gate["allowed"]:
        ...
        if args.as_json:
            ...
            print(f"[probe-rs {args.action}] failed - {blocked['error']['message']}", file=sys.stderr)
        sys.exit(1)
