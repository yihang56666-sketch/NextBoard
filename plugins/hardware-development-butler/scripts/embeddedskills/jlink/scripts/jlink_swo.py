...
def main() -> None:
    ...
    target = project_config.get("device") or get_state_entry(state, "last_debug").get("device") or get_state_entry(state, "last_flash").get("device") or "jlink-target"
    safety_cli.require_gate(
        action="swo",
        token=args.confirm_token,
        target=str(target),
        probe=str(config.get("serial") or config.get("exe") or "J-Link"),
        voltage=args.voltage,
        current_limit=args.current_limit,
        recovery=args.recovery,
        external_loads=args.external_loads,
        artifact=args.safety_artifact or "swo-viewer",
